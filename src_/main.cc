...
int main(void)
{
    zhanmm::SphShmget shmg(KEY,  sizeof(Session)+conf_size, NULL);
    void * ptr = shmg.attach();
    memset(ptr, 0, sizeof(Session) + conf_size);
    pid_t pid = fork();
    if (0 == pid) {
        do_child();
    } else {
...
void do_child()
{
    zhanmm::SphShmget shmg(KEY, sizeof(Session) + conf_size);
    void  *tmp = shmg.attach();
    Session *sess = static_cast<Session*>(tmp);
    sess->shared = static_cast<char*>(tmp) + sizeof(Session);
    sess->size = conf_size;
    sess->threadNums = 4; 
    zhanmm::SphEpoller epoll;
    zhanmm::SphThreadPool  threadpool(sess, epoll);
